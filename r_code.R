...
require(glmnet)
set.seed(12)
rawdat<-read.csv(file="C:/Users/vineela/Downloads/Train_music.csv")
rawdat
...
#test1 <- read.csv(file="C:/Users/vineela/Downloads/Test.csv")
...
for(i in 1:length(lasso_alphas)) {
  ...
  ols_mse_lat_new = sum((y_lat_test - yhatols_lat)^2+(y_long_test - yhatols_long)^2)/nrow(xdat_test)
  ridge_mse_lat_new = sum((y_lat_test - yhatridge_lat)^2+(y_long_test - yhatridge_long)^2)/nrow(xdat_test)
  lasso_mse_lat_new = sum((y_lat_test - yhatlasso_lat)^2+(y_long_test - yhatlasso_long)^2)/nrow(xdat_test)
  elastic_mse_lat_new = sum((y_lat_test - yhatelastic_lat)^2+(y_long_test - yhatelastic_long)^2)/nrow(xdat_test)
...
plot(x=observed_lat, y=predicted_lat, type="p",main="Predicted vs. Observed Latitude",xlab="Observed Latitude", ylab="Predicted Latitude")

#residuals vs predicted

plot(x=predicted_lat, y=residuals_lat, type="p", main="Residual vs. Predicted Latitude",xlab="Predicted Latitude", ylab="Residual")
